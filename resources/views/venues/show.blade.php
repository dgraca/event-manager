<x-app-layout>
    ...
    <div class="intro-y mt-8 flex flex-col items-center sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">{{ __('Venue Details') }}</h2>
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            <x-base.button
                class="mr-2 shadow-md ml-auto sm:ml-0"
                variant="primary"
                href="{{ route('venues.edit', $venue->slug) }}"
                as="a"
                >
                <x-base.lucide
                        class="mr-2 h-4 w-4"
            ...
            <form method="POST" action="{{ route('venues.destroy', $venue->slug) }}" id="delete-record-form">
                @csrf
                @method('DELETE')
            </form>
        </div>
