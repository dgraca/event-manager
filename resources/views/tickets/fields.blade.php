...
<!-- Name Field -->
<div class="mb-3">
    <x-base.form-label for="name">{{ $tickets->getAttributeLabel('name') }}</x-base.form-label>
    <x-base.form-input
        class="w-full {{ ($errors->has('name') ? 'border-danger' : '') }}"
        id="name"
        name="name"
        :value="old('name', $tickets->name ?? '')"
        type="text"
    />
    @error('name')
        <div class="mt-2 text-danger">{{ $message }}</div>
...
<!-- Description Field -->
<div class="mb-3">
    <x-base.form-label for="description">{{ $tickets->getAttributeLabel('description') }}</x-base.form-label>
    <x-base.form-textarea
        class="w-full {{ ($errors->has('description') ? 'border-danger' : '') }}"
        id="description"
        name="description"
        rows="5"
    >{{ old('description', $tickets->description ?? '') }}</x-base.form-textarea>
    @error('description')
        <div class="mt-2 text-danger">{{ $message }}</div>
    @enderror
</div>
...
<!-- Max Check In Field -->
<div class="mb-3">
    <x-base.form-label for="max_check_in">{{ $tickets->getAttributeLabel('max_check_in') }}</x-base.form-label>
    <x-base.form-input
        class="w-full {{ ($errors->has('max_check_in') ? 'border-danger' : '') }}"
        id="max_check_in"
        name="max_check_in"
        :value="old('max_check_in', $tickets->max_check_in ?? '')"
        type="number"
        step="1"
    />
    @error('max_check_in')
...
<div class="mb-3">
    <x-base.form-label for="max_tickets_per_order">{{ $tickets->getAttributeLabel('max_tickets_per_order') }}</x-base.form-label>
    <x-base.form-input
...
<!-- Price Field -->
<div class="mb-3">
    <x-base.form-label for="price">{{ $tickets->getAttributeLabel('price') }}</x-base.form-label>
    <x-base.form-input
        class="w-full {{ ($errors->has('price') ? 'border-danger' : '') }}"
        id="price"
        name="price"
        :value="old('price', $tickets->price ?? '')"
        type="number"
        step="1"
    />
    @error('price')
...
<!-- Currency Field -->
<div class="mb-3">
    <x-base.form-label for="currency">{{ $tickets->getAttributeLabel('currency') }}</x-base.form-label>
    <x-base.form-input
        class="w-full {{ ($errors->has('currency') ? 'border-danger' : '') }}"
        id="currency"
        name="currency"
        :value="old('currency', $tickets->currency ?? '')"
        type="text"
    />
    @error('currency')
        <div class="mt-2 text-danger">{{ $message }}</div>
