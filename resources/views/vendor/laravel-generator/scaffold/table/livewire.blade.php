...
class {{ $config->modelNames->plural }}Table extends Component implements HasForms, HasTable
{
    ...
    {
        $newModel = new {{ $config->modelNames->name }}();
        return $table
            ...
            ->actions([
                Action::make('edit')
                ->label(__('Update'))
                ->url(fn ({{ $config->modelNames->name }} $record): string => route('{{ $config->modelNames->dashedPlural }}.edit', ['{{ $config->modelNames->snake }}' => $record]))
            ...
            ->defaultSort('{{ $config->primaryName }}', 'desc')
            ->recordUrl(
                fn (Model $record): string => route('{{ $config->modelNames->dashedPlural }}.show', ['{{ $config->modelNames->snake }}' => $record]),
            )
            //->striped()
