<x-app-layout>
    @section('breadcrumbs')
        {{ Breadcrumbs::render('access-tickets.show', $accessTickets) }}
    @endsection
    <div class="intro-y mt-8 flex flex-col items-center sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">{{ __('Access Tickets Details') }}</h2>
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            <x-base.button
                class="mr-2 shadow-md ml-auto sm:ml-0"
                variant="primary"
                href="{{ route('access-tickets.edit', $accessTickets) }}"
                as="a"
                >
                <x-base.lucide
                        class="mr-2 h-4 w-4"
            ...
            <form method="POST" action="{{ route('access-tickets.destroy', $accessTickets) }}" id="delete-record-form">
                @csrf
                @method('DELETE')
            </form>
        </div>
    ...
    <div class="intro-y box mt-3 p-5">
        <dl class="space-y-4">
            @include('access_tickets.show_fields')
        </dl>
    </div>
</x-app-layout>
