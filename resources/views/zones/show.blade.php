<x-app-layout>
    @section('breadcrumbs')
        {{ Breadcrumbs::render('zones.show', $zone) }}
    @endsection
    <div class="intro-y mt-8 flex flex-col items-center sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">{{ __('Zone Details') }}</h2>
        <div class="mt-4 flex w-full sm:mt-0 sm:w-auto">
            ...
            <form method="POST" action="{{ route('zones.destroy', $zone) }}" id="delete-record-form">
                @csrf
                @method('DELETE')
            </form>
        </div>
