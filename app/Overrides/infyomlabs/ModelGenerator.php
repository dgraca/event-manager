...
class ModelGenerator extends BaseGenerator
{
    ...
    public function generate()
    {
        ...
        //generate comments for the model
        $this->config->command->call('ide-helper:models', ['model' => ["App\Models\\".$this->config->modelNames->name], '--write' => true, ]);
        //generate the ide-helper
        $this->config->command->call('ide-helper:generate');
    ...
    public function variables(): array
    {
        return [
            ...
            'haveStatusField'  => $this->checkHaveStatusField(), // added by me
            'attributeLabels'  => implode(','.infy_nl_tab(1, 2), $this->generateAttributeLabels()), // added by me
        ];
    }
    ...
    //added by me
    protected function checkHaveStatusField(): bool
    {
        $haveStatus = false;

        if (isset($this->config->fields) && !empty($this->config->fields)) {
            foreach ($this->config->fields as $field) {
                //TODO usar o campo description e o campo dbType para ver se é um select e criar depois as contastantes e para todos os selects
    ...
    //added by me
    protected function generateAttributeLabels(): array
    {
        $attributeLabels = [];

        if (isset($this->config->fields) && !empty($this->config->fields)) {
            foreach ($this->config->fields as $field) {
                //$attributeLabels[$field->name] = "__('".$field->getTitle()."'),";
                $attributeLabels[] = "'".$field->name."' => __('".$field->getTitle()."')";
            }
        }
