...
//        }
    ...
    {
        //edited by me
        if (false && $this->config->tableType === 'livewire') {
            return;
        }
        ...
        switch ($this->config->tableType) {
            ...
            case 'livewire': // added by me
                $templateData = $this->generateLivewireTableBody();
                break;
    ...
    //created by me
    protected function generateLivewireTableBody(): string
    {
        return view($this->templateViewPath.'.scaffold.table.livewire.table')->render();
    }
    ...
    protected function generateFields()
    {
        ...
        foreach ($this->config->fields as $field) {
            ...
            //added by me to force status to be a select input
            if (strpos($field->name, 'status') !== false) {
                $field->htmlType = 'select';
                //TODO usar o campo description e o campo dbType para ver se é um select
            }
...
//            }
        ...
        g_filesystem()->createFile($this->path.'fields.blade.php', implode(infy_nls(1), $htmlFields));
